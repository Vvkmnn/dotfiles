...
class StashExplorer {
    constructor(context, git) {
        ...
        vscode_1.commands.registerCommand('gitlens.stashExplorer.toggle', this.toggle, this);
    ...
    refresh() {
        if (!this.git.config.stashExplorer.enabled)
            return;
        this._onDidChangeTreeData.fire();
    }
    toggle() {
        const cfg = vscode_1.workspace.getConfiguration().get(configuration_1.ExtensionKey);
        vscode_1.workspace.getConfiguration(configuration_1.ExtensionKey).update('stashExplorer.enabled', !cfg.stashExplorer.enabled, true);
    }
    openChanges(node) {
