...
- Escalation ladder (cheapest-first): single hard turn → suggest prefixing **`ultrathink`** (per-turn keyword: adds an in-context deeper-reasoning instruction, API effort unchanged, model-agnostic) · sustained hard work → `/effort xhigh` · only if effort is maxed and Opus still stalls → `/model fable` (credits, ask first). A keyword/effort bump is cheaper than a model tier bump. (`ultracode` is a heavier session setting — `xhigh` + dynamic-workflow orchestration, v2.1.203+ — not a per-turn keyword.)
