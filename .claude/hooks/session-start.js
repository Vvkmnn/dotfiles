...
// ============================================================================
// Maintenance reminder (if >7 days since last improve-claude run)
// Reads .last-maintenance (current mechanism), falls back to legacy .last-update-check
// ============================================================================
const maintFile = path.join(os.homedir(), '.claude', '.last-maintenance');
const legacyFile = path.join(os.homedir(), '.claude', '.last-update-check');

try {
  const sourceFile = fs.existsSync(maintFile) ? maintFile : legacyFile;
  const lastCheck = fs.existsSync(sourceFile)
    ? new Date(fs.readFileSync(sourceFile, 'utf8').trim())
    : new Date(0); // Never run

  const daysSinceCheck = (Date.now() - lastCheck.getTime()) / (1000 * 60 * 60 * 24);

  if (daysSinceCheck > 7) {
    const reminder = `<system-reminder>It has been ${Math.floor(daysSinceCheck)} days since last Claude Code maintenance. Consider running the improve-claude skill (health + audit phases) to catch drift and new features.</system-reminder>`;
