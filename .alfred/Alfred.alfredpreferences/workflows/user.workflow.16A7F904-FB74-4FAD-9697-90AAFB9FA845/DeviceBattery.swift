    ...
    struct Device {
        ...
        var alfredItem: String {
            if isAirPods, let levels = airPodsLevels {
                return "{\"title\":\"\(name)\",\"subtitle\":\"􀹬 \(levels.case.stripped) 􀲎 \(levels.left.stripped) 􀲍 \(levels.right.stripped)\",\"valid\":\"false\",\"icon\":{\"path\":\"icons/airpodspro.png\"}}"
            }
        ...
        for entry in deviceEntries {
            if entry.contains("AirPods") {
                if let nameMatch = entry.firstMatch(of: #/^\s+(.+):\n/#),
                   ...
                   let leftBattery = entry.firstMatch(of: #/Left Battery Level:\s+(\d+\s?%)/#),
                   let rightBattery = entry.firstMatch(of: #/Right Battery Level:\s+(\d+\s?%)/#)
                {
                    let name = String(nameMatch.output.1)
                    let address = String(addressMatch.output.1).lowercased()
                    let caseBattery: String = entry.firstMatch(of: #/(?:Case Battery Level:\s)(\d?\d?\d\s?\%)/#).map({ String($0.output.1) }) ?? "n/A"
