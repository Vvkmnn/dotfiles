...
" Add minpac to the RTP
function! packages#setup() abort
    packadd minpac
    call minpac#init() " Initialize

    call minpac#add('dracula/vim', {'type': 'start'}) 
    call minpac#add('k-takata/minpac', {'type': 'opt'}) 
    " minpac must have {'type': 'opt'} so that it can be loaded with `packadd`.

endfunction
" function! packages#DeinSetup() abort
" 
"     if dein#load_state('~/.config/nvim/pack')
"         call dein#begin('~/.config/nvim/pack')
" 
"         " Package Plugins {{{
