...
SILVER=0xffB0B7C0  # Silver color for arrows
...
if [ -f "$PREV_FILE" ]; then
    ...
    if [ $DOWN_KB -eq 0 ]; then
        ...
        DOWN_COLOR=0xffC0C0C0  # Silver for moderate
    else
    ...
    if [ $UP_KB -eq 0 ]; then
        ...
        UP_COLOR=0xffC0C0C0  # Silver
    else
        UP_COLOR=0xffE74C3C  # Red
    fi
    
    # Update items - arrows with silver color, numbers change color based on activity
    # Using separate color codes for arrow (always silver) and numbers
    ...
    # No previous data - show minimal sparklines with arrows, silver color
    sketchybar --set network_down label="↓ 0 ····" label.color=$SILVER label.font="SF Pro:Regular:13.0" label.drawing=on \
               --set network_up label="↑ 0 ····" label.color=$SILVER label.font="SF Pro:Regular:13.0" label.drawing=on
fi
